...
namespace Terran {
	...
	void OpenGLShader::UploadUniformInt2(const std::string& name, const glm::vec4& value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
		glUniform2i(location, value.x, value.y);
	}

	void OpenGLShader::UploadUniformInt3(const std::string& name, const glm::vec4& value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::Preprocess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			TR_CORE_ASSERT(eol != std::string::npos, "Shader syntax error");
			size_t begin = pos + typeTokenLength + 1;
			std::string type = source.substr(begin, eol - begin);
			TR_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type");
	...
	void OpenGLShader::Compile(std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		TR_CORE_ASSERT(shaderSources.size() <= 2, "Only support compiling 2 shaders");
